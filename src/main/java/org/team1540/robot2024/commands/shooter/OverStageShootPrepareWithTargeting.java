...
public class OverStageShootPrepareWithTargeting extends ParallelCommandGroup {
    public OverStageShootPrepareWithTargeting(XboxController controller, Drivetrain drivetrain, Shooter shooter) {
        addCommands(
                new DriveWithTargetingCommand(drivetrain,controller, ()-> AprilTagsCrescendo.getInstance().getTag(AprilTagsCrescendo.Tags.SPEAKER_CENTER).toPose2d().plus(new Transform2d(0,-2 * (DriverStation.getAlliance().orElse(null) == DriverStation.Alliance.Blue ? -1 : 1),new Rotation2d()))),
